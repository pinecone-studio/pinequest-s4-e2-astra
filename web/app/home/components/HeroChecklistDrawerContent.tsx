...
import { Bell, BellOff, Calendar, Check, CheckSquare, Clock, Plus, Trash2 } from "lucide-react";
...
export default function HeroChecklistDrawerContent() {
  ...
  return (
    <>
        ...
        {checklistCategories.map((cat) => (
          <button
            ...
            className={`shrink-0 rounded-full px-3 py-1.5 text-[11px] font-black transition-all ${
              checklist.selectedCategory === cat ? "bg-[#0A4429] text-white shadow-sm" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
            }`}
          >
            {cat}
          </button>
      ...
      <form onSubmit={checklist.addItem} className="flex shrink-0 gap-2 bg-white p-5 pb-3">
        <input
          type="text"
          value={checklist.inputValue}
          onChange={(event) => checklist.setInputValue(event.target.value)}
          placeholder={`"${checklist.selectedCategory}" ангилалд нэмэх...`}
        ...
        />
        <button type="submit" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#0A4429] text-white shadow-sm transition-colors hover:bg-[#083520]">
      ...
      <div className="shrink-0 border-b border-gray-100 bg-white px-5 pb-4">
        {checklist.isAlarmTriggered ? (
          <div className="flex animate-pulse items-center justify-between rounded-2xl border border-rose-100 bg-rose-50 p-3 shadow-sm">
            <span className="text-xs font-black tracking-tight text-rose-800">ЦАГ БОЛЛОО: Юмаа бэлдээрэй!</span>
            <button
              type="button"
          ...
          <div className="flex items-center justify-between gap-1.5 rounded-2xl border border-gray-100 bg-gray-50/80 p-2.5">
            <div className="flex min-w-0 flex-1 items-center gap-1">
              <div className="flex min-w-0 flex-1 items-center gap-1 rounded-lg border border-gray-100 bg-white px-1.5 py-1">
                ...
                <input
                  ...
                  onChange={(event) => checklist.setAlarmDate(event.target.value)}
              ...
              <div className="flex shrink-0 items-center gap-1 rounded-lg border border-gray-100 bg-white px-1.5 py-1">
                ...
                <input
                  ...
                  disabled={checklist.isAlarmSet}
                  onChange={(event) => checklist.setAlarmTime(event.target.value)}
            ...
            <button
              ...
              className={`flex h-7 shrink-0 items-center gap-1 rounded-xl px-2.5 text-[10px] font-black transition-all ${
                checklist.isAlarmSet ? "bg-emerald-600 text-white shadow-sm" : "bg-gray-200 text-gray-600 hover:bg-gray-300"
              }`}
            >
              {checklist.isAlarmSet ? <Bell className="h-3 w-3 animate-pulse" /> : <BellOff className="h-3 w-3" />}
            ...
            {checklist.visibleItems.map((item) => (
              <div
                ...
                className={`flex items-center justify-between rounded-2xl border p-3.5 transition-all ${
                  item.checked ? "border-gray-100 bg-gray-50/60" : "border-gray-100 bg-white shadow-sm"
                }`}
              >
                <button type="button" onClick={() => checklist.toggleCheck(item.id)} className="flex flex-1 items-center gap-3 text-left">
                  <span
                    className={`flex h-4 w-4 items-center justify-center rounded-full border transition-all ${
                      item.checked ? "border-[#0A4429] bg-[#0A4429] text-white" : "border-gray-300 bg-white"
                    }`}
                  >
                    {item.checked && <Check className="h-2.5 w-2.5 stroke-[3]" />}
                  </span>
                  <span className={`text-xs font-medium transition-all ${item.checked ? "text-gray-400 line-through" : "text-gray-700"}`}>
                    {item.text}
                  </span>
                </button>
                <button type="button" onClick={() => checklist.deleteItem(item.id)} className="p-1 text-gray-400 transition-colors hover:text-red-500">
